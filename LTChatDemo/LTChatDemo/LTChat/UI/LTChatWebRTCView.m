...
/** 摄像头按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *cameraBtn;
/** 扬声器按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *loudspeakerBtn;
/** 邀请成员按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *inviteBtn;
/** 消息回复按钮 */
@property (strong, nonatomic)   UIButton                *msgReplyBtn;
/** 收到视频通话时，语音接听按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *voiceAnswerBtn;
/** 挂断按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *hangupBtn;
/** 接听按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *answerBtn;
/** 收起按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *packupBtn;
/** 视频通话缩小后的按钮 */
@property (strong, nonatomic)   UIButton                *videoMicroBtn;
/** 音频通话缩小后的按钮 */
@property (strong, nonatomic)   LTChatWebRTCButton               *microBtn;
/** 遮罩视图 */
...
- (void)cameraClick{
    ...
    if (self.localCamera) {
        ...
        [self initUIForVideoCaller];
        // 在这里添加 开启本地视频采集 的代码
        [[NSNotificationCenter defaultCenter] postNotificationName:kLTCHAT_WEBRTC_CAPTURE_NOTIFICATION object:@{@"videoCapture":@(YES)}];
        // 对方和本地都开了摄像头
        ...
        // 在这里添加 关闭本地视频采集 的代码
        [[NSNotificationCenter defaultCenter] postNotificationName:kLTCHAT_WEBRTC_CAPTURE_NOTIFICATION object:@{@"videoCapture":@(NO)}];
        if (self.oppositeCamera) {
            // 本地未开，对方开了摄像头
            [self clearAllSubViews];
        ...
        if (self.answered) {
            [UIView animateWithDuration:1.0 animations:^{
                ...
                if (self.oppositeCamera && self.localCamera) {
                    ...
                    [self updateFrameOfRemoteView:CGRectMake(0, 0, kMicVideoW, kMicVideoH)];
                } else if (!self.oppositeCamera && self.localCamera) {
                    //                    self.ownImageView.frame = CGRectMake(0, 0, kMicVideoW, kMicVideoH);
                    [self updateFrameOfLocalView:CGRectMake(0, 0, kMicVideoW, kMicVideoH)];
                } else {
                    //                    self.adverseImageView.frame = CGRectMake(0, 0, kMicVideoW, kMicVideoH);
                    [self updateFrameOfRemoteView:CGRectMake(0, 0, kMicVideoW, kMicVideoH)];
                }
    ...
    if (self.answered) {
        [UIView animateWithDuration:1.0 animations:^{
            ...
            if (!self.oppositeCamera && self.localCamera) {
                ...
                [self updateFrameOfLocalView:[UIScreen mainScreen].bounds];
            } else {
                //                self.adverseImageView.frame = [UIScreen mainScreen].bounds;
                [self updateFrameOfRemoteView:[UIScreen mainScreen].bounds];
            }
...
{
    if (!_muteBtn) {
        _muteBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"静音" imageName:@"icon_avp_mute" isVideo:_isVideo];
...
- (LTChatWebRTCButton *)cameraBtn
{
    if (!_cameraBtn) {
        _cameraBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"摄像头" imageName:@"icon_avp_video" isVideo:_isVideo];
...
{
    if (!_loudspeakerBtn) {
        _loudspeakerBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"扬声器" imageName:@"icon_avp_loudspeaker" isVideo:_isVideo];
...
- (LTChatWebRTCButton *)inviteBtn
{
    if (!_inviteBtn) {
        _inviteBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"邀请成员" imageName:@"icon_avp_invite" isVideo:_isVideo];
...
- (LTChatWebRTCButton *)packupBtn
{
    if (!_packupBtn) {
        _packupBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"收起" imageName:@"icon_avp_reduce" isVideo:_isVideo];
...
- (UIButton *)msgReplyBtn
{
    if (!_msgReplyBtn) {
        if (self.isVideo) {
            _msgReplyBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"消息回复" noHandleImageName:@"icon_av_reply_message_normal"];
        } else {
...
- (LTChatWebRTCButton *)voiceAnswerBtn
{
    if (!_voiceAnswerBtn) {
        _voiceAnswerBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"语音接听" noHandleImageName:@"icon_av_audio_receive_normal"];
...
- (LTChatWebRTCButton *)microBtn
{
    if (!_microBtn) {
        _microBtn = [[LTChatWebRTCButton alloc] initWithTitle:@"等待中" noHandleImageName:@"icon_av_audio_micro_normal"];
